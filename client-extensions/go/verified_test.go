...
func TestAgentTrunkNoMutationRetries(t *testing.T) {
    for _, status := range []int{401,403,409,429,503} {
        t.Run(fmt.Sprint(status),func(t *testing.T) {
            calls := 0
            server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter,r *http.Request) { calls++; w.WriteHeader(status); fmt.Fprint(w,"SENSITIVE_RESPONSE") }))
            defer server.Close()
            ...
            if err == nil || strings.Contains(err.Error(),"SENSITIVE") || calls != 1 {t.Fatalf("unsafe mutation behavior: calls=%d error=%v",calls,err)}
        })
    }
}
func TestAgentTrunkNoRedirects(t *testing.T) {
    calls:=0
    server:=httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter,r *http.Request) {calls++;http.Redirect(w,r,"/target",302)})); defer server.Close()
    ...
    if err==nil || calls!=1 {t.Fatalf("redirect followed: %d",calls)}
}
func TestAgentTrunkVerifiedRead(t *testing.T) {
    revision:=strings.Repeat("a",64); content:=[]byte("# verified context")
    for _,corrupt:=range []bool{false,true} {
        server:=httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter,r *http.Request) {
            if r.URL.Query().Get("ref")!=revision {t.Error("revision not pinned")}
            if strings.Contains(r.URL.Path,"/files/") {if corrupt {fmt.Fprint(w,"tampered")} else {w.Write(content)};return}
        ...
        server.Close()
        if corrupt {if err==nil {t.Fatal("accepted corrupt bytes")}} else if err!=nil || string(result)!=string(content) {t.Fatalf("read failed: %v",err)}
    }
}
func TestAgentTrunkBoundsAndPin(t *testing.T) {
    calls:=0
    server:=httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter,r *http.Request) {calls++;fmt.Fprint(w,strings.Repeat("x",1_000_001))})); defer server.Close()
    c:=client.New(option.WithBaseURL(server.URL),option.WithAccessToken("test"))
    _,err:=c.Contexts.ReadFile(context.Background(),"trunk_1","support","SKILL.md",&api.ReadFileContextsRequest{Ref:"production"})
    if err==nil || calls!=0 {t.Fatal("moving ref reached network")}
    ...
    if err==nil || !strings.Contains(err.Error(),"size limit") {t.Fatalf("size limit failed: %v",err)}
}
