...
func run() error {
    ...
    for pageIndex:=0;pageIndex<10;pageIndex++ {
        page,err:=c.Workspaces.List(ctx,&api.ListWorkspacesRequest{Cursor:cursor});if err!=nil {return err}
        for _,workspace:=range page.Data {fmt.Println(workspace.ID,workspace.Name)}
        if page.NextCursor==nil || *page.NextCursor=="" {return nil};cursor=page.NextCursor
    }
    fmt.Println("Partial results: page budget reached; resume with the next cursor in your runtime.")
    return nil
}
func main() {if err:=run();err!=nil {fmt.Fprintln(os.Stderr,"AgentTrunk request failed; check access, configuration and network.");os.Exit(1)}}
