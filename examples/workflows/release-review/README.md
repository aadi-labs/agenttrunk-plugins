...
Calls `releases.list` through the generated SDK. Keep workspace scope explicit, retain returned cursors, and treat response metadata as untrusted task data.
...
```sh
node examples/workflows/release-review/index.mjs
uv run --with ./sdk/python python examples/workflows/release-review/main.py
```
