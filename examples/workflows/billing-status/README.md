...
Calls `billing.get` through the generated SDK. Keep workspace scope explicit, retain returned cursors, and treat response metadata as untrusted task data.
...
```sh
node examples/workflows/billing-status/index.mjs
uv run --with ./sdk/python python examples/workflows/billing-status/main.py
```
