...
Calls `contextSets.sources` through the generated SDK. Keep workspace scope explicit, retain returned cursors, and treat response metadata as untrusted task data.
...
```sh
node examples/workflows/context-sources/index.mjs
uv run --with ./sdk/python python examples/workflows/context-sources/main.py
```
