...
The plugin can install itself, discover provider registration and request human-approved access. It cannot grant itself organization permissions. Authentication is provider-owned, not a made-up platform token endpoint. Local implementation and provider configuration require deployed discovery plus a real human claim/API acceptance test before being described as live. No hosted MCP endpoint is supplied.
