...
<CardGroup cols={2}>
  <Card title="Connect your first agent" href="/quickstart">Sign in, upload a prompt to staging, and read back the exact version.</Card>
  <Card title="Let your agent set up" href="/guides/agent-setup">Give your agent a setup task with a human approval handoff.</Card>
  <Card title="Build with the SDK" href="/sdks/typescript">Load verified context in your own agent runtime.</Card>
  <Card title="Explore the API" href="/api-overview">Find endpoint schemas, authentication, and request conventions.</Card>
</CardGroup>

- [Connect an agent](https://docs.agenttrunk.ai/quickstart): Install the CLI, get human approval, and select a workspace.
- [Upload context](https://docs.agenttrunk.ai/publishing): Publish a skill or prompt to staging.
- [Find and read context](https://docs.agenttrunk.ai/retrieval): Search compact metadata, then fetch only the files you need.
- [Review and release](https://docs.agenttrunk.ai/releases): Compare revisions and promote a scope to production.
- [Authentication](https://docs.agenttrunk.ai/authentication): Connect without sharing a human's password.
- [Integrations](https://docs.agenttrunk.ai/integrations): CLI, SDKs, Claude Code, Codex, and portable skills.
- [Troubleshooting](https://docs.agenttrunk.ai/troubleshooting): Recover from access errors and uncertain writes.
...
You can store memory schemas and explicit context files. AgentTrunk is not an automatic memory extractor, a vector database, or an agent execution service. Connect through [operational MCP](/guides/mcp), the CLI, SDK or REST API.
