  ...
  module Internal
    module Http
      ...
      class RawClient
        ...
        def initialize(base_url:, max_retries: 0, timeout: 60.0, headers: {}, overridable_headers: [], auth_provider: nil)
          @base_url = base_url
          ...
          @default_headers = {
            ...
            "X-Fern-SDK-Name": "agenttrunk",
            "X-Fern-SDK-Version": "0.1.0"
          }.merge(headers)
          @overridable_headers = overridable_headers.to_set { |name| name.to_s.downcase }
        end

        # @param request [AgentTrunk::Internal::Http::BaseRequest] The HTTP request.
        ...
        def send(request)
          ...
          AgentTrunk::Safety.validate!(url, @default_headers.merge(auth_headers))
          attempt = 0
          response = nil
          ...
          loop do
            ...
            response = AgentTrunk::Safety.request(conn, http_request, url)

            break unless %w[GET HEAD].include?(request.method) && should_retry?(response, attempt)
