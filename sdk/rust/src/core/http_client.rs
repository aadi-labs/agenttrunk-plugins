...
impl ByteStream {
    ...
    pub async fn try_next(&mut self) -> Result<Option<bytes::Bytes>, ApiError> {
        match self.inner.next().await {
            Some(Ok(bytes)) => { self.received += bytes.len(); if self.received > 1_000_000 { return Err(ApiError::InvalidHeader); } Ok(Some(bytes)) },
...
impl Stream for ByteStream {
    ...
    fn poll_next(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Option<Self::Item>> {
        match self.inner.as_mut().poll_next(cx) {
            Poll::Ready(Some(Ok(bytes))) => { self.received += bytes.len(); if self.received > 1_000_000 { return Poll::Ready(Some(Err(ApiError::InvalidHeader))); } Poll::Ready(Some(Ok(bytes))) },
...
impl HttpClient {
    ...
    async fn send_request(
        ...
        if let Some(executor) = &self.executor {
            ...
            self.apply_custom_headers(&mut req, options)?;
            if req.url().path().starts_with("/v1/") && !req.headers().get("authorization").and_then(|v|v.to_str().ok()).map(|s|s.starts_with("Bearer ") && s.len()>7).unwrap_or(false) { return Err(ApiError::InvalidHeader); }
    ...
    async fn execute_with_retries(
        ...
        let max_retries = if request.method() == Method::GET || request.method() == Method::HEAD { max_retries.min(5) } else { 0 };
        ...
        for attempt in 0..=max_retries {
            ...
            match self.client.execute(cloned_request).await {
                ...
                Ok(response) => {
                    let status_code = response.status().as_u16();
                    return Err(ApiError::from_response(status_code, None));
                }
    ...
    async fn bounded_text(mut response: Response) -> Result<String, ApiError> {
        ...
        T: DeserializeOwned,
    {
        ...
        let text = Self::bounded_text(response).await?;
